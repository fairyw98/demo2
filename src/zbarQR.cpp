...
void zbarscanner(cv_bridge::CvImagePtr cv_ptr,std::stringstream &stringStr)
{
...
class ImageConverter
{
  ...
  public:
    ImageConverter():it(nh)
    {
        ...
        image_pub=it.advertise("zbarQR_image",1);
        string_pub=nh.advertise<std_msgs::String>("zbarQR_string",1000);
	...
	//stringStr << count;
        ...
        zbarscanner(cv_ptr,stringStr);
	msg_str.data = stringStr.str();
	ROS_INFO("%s",msg_str.data.c_str());
	if(msg_str.data.size()>2){
	string_pub.publish(msg_str);
